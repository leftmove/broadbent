...
export function ChatInput({
  ...
  const handleSubmit = async (message: string) => {
    ...
    try {
      ...
      // Prepare message history with tool context
      const messageHistory: Array<any> = [];
      ...
      messages
        ...
        .forEach((msg) => {
          ...
          if (msg.toolResults && msg.toolResults.length > 0) {
            msg.toolResults.forEach((tr: any) => {
              messageHistory.push({
                role: "tool",
      ...
      // Generate AI response - ensure web search is enabled if model supports it and user has it enabled
      const shouldEnableWebSearch =
        searchEnabled && currentModel.capabilities?.tool;
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="flex justify-between items-center px-4 py-3 pr-4 border-b transition-colors duration-200 border-border/30">
          <button
